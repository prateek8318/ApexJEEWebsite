...
const featuresData = [
  ...
  },
  {
    ...
    desc: "Complete chapter-wise revision notes, formula sheets, and video lectures — professionally written and exam-ready. You don't need to make them yourself.",
    iconImg: VideoImg,
    ...
    <section className="bg-[#F4F6FB] py-16 px-8 md:px-16 lg:px-24">
      <div className="max-w-[1800px] mx-auto">
        ...
        <div className="text-center mb-16">
          ...
          <h2 className="text-4xl md:text-5xl font-bold text-[#0F172A] tracking-tight mb-3">
            Everything you need,{" "}

          </h2>
          <h3 className="text-4xl md:text-5xl italic text-[#F59E0B] font-serif font-medium mb-4">
            nothing you don&apos;t
          </h3>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-10">
          {featuresData.map((f, index) => {
            return (
              ...
              >
                <div>
                  <div className={`w-16 h-16 rounded-xl flex items-center justify-center mb-6 transition-colors duration-300 ${f.iconBg} group-hover:bg-[rgba(255,255,255,0.08)]`}>
                    <Image src={f.iconImg} alt={f.title} width={36} height={36} className="object-contain" />
                  </div>
                  <h3 className="text-2xl font-bold font-serif mb-4 text-[#0F172A] group-hover:text-white transition-colors duration-300">
                    {f.title}
                  </h3>
                  <p className="text-base leading-relaxed mb-6 text-[#64748B] group-hover:text-[#94A3B8] transition-colors duration-300">
                    {f.desc}
                  </p>
                </div>
                <ul className="space-y-2.5">
                  {f.bullets.map((bullet, i) => (
                    <li key={i} className="flex items-center gap-2 text-sm text-[#475569] group-hover:text-[#E2E8F0] transition-colors duration-300">
