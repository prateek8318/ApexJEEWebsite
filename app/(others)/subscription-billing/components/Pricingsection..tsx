...
export default function PricingSection() {
  ...
  const handleSubscribe = (plan: any) => {
    toast.info("Coming Soon", { description: `Payment gateway integration for ${plan.name} is in progress.` });
  };

  if (isLoading) {
    return (
      <section id="pricing" className="bg-[#F0F2F8] py-5 px-6 md:px-12 lg:px-20 min-h-[400px] flex items-center justify-center">
        <p className="text-[#F5A623] font-semibold text-lg animate-pulse">Loading available plans...</p>
      </section>
    );
  }
              ...
              >
                ...
                <div className="p-5 flex flex-col gap-5 flex-1">
                  ...
                  <div>
                    ...
                    {priceNote && (
                      <p className={`text-xs mt-1 ${isHighlight ? 'text-white/40' : 'text-[#9CA3AF] group-hover:text-white/40'}`}>
                        {priceNote}
                      </p>
                    )}
                  </div>
                  ...
                  <Button
                    ...
                    className={`w-full mt-2 text-sm h-10 rounded-lg font-semibold transition-all duration-300 ${isHighlight ? 'bg-[#F5A623] hover:bg-[#F5A623]/90 text-black border-none' : 'border-[#0D1525] text-[#0D1525] group-hover:bg-[#F5A623] group-hover:text-black group-hover:border-[#F5A623]'}`}
                  >
