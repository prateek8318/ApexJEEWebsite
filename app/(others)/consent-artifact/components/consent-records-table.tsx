...
import { ColumnDef, VisibilityState, flexRender, getCoreRowModel, getFilteredRowModel, getPaginationRowModel, SortingState, getSortedRowModel, useReactTable, PaginationState } from "@tanstack/react-table";
import { Input } from "@components/ui/input";
import { useDebounce } from "react-use";

type ConsentRecordsTableProps = {
	data: ConsentRecordType[];
	columns: ColumnDef<ConsentRecordType>[];
};

const ConsentRecordsTable = ({ data, columns }: ConsentRecordsTableProps) => {
                        ...
                        <TableBody>
                            {table.getRowModel().rows?.length ? (
                                table.getRowModel().rows.map(row => (
                                    <TableRow key={row.id} data-state={row.getIsSelected() && "selected"} className="border-gray-50 hover:bg-gray-50/30 transition-colors">
...
export default ConsentRecordsTable;
