...
const AdminRegisterForm = () => {
  ...
  const onRegister = async (values: RegisterFormValues) => {
    ...
    // Use FormData for Admin registration
  ...
  const { mutate, isPending } = useMutation({
    ...
    onSuccess: (data: any) => {
      ...
      router.push(`/auth/verify?email=${encodeURIComponent(data?.email || "")}&type=admin`);
    },
      ...
      >
        ...
        <FormField
          ...
          render={({ field }) => (
            <FormItem className="space-y-1.5">
              <FormLabel className="text-[12.5px] font-medium text-gray-700">
                Profile Image <span className="font-normal text-gray-400">(Optional)</span>
              </FormLabel>
              <FormControl>
                <FileInput
                  accept={{ 'image/*': [] }}
